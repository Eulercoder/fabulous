...
class InvalidServiceDirectory(Exception):
    def __init__(self, servicedirectory):
        message = "Unable to find Service dir {0}".format(servicedirectory)
        super(InvalidServiceDirectory, self).__init__(message)
...
def init_services(servicedirectory, Services_to_load=None):
    if servicedirectory and not os.path.isdir(servicedirectory):
        raise InvalidServiceDirectory(servicedirectory)

    if not servicedirectory:
        servicedirectory = DIR("services")

    logger.debug("servicedirectory: {0}".format(servicedirectory))

    if os.path.isdir(servicedirectory):
        Servicefiles = glob(os.path.join(servicedirectory, "[!_]*.py"))
        Services = strip_extension(os.path.basename(p) for p in Servicefiles)
    else:
        # we might be in an egg; try to get the files that way
        logger.debug("trying pkg_resources")
        import pkg_resources
        try:
            Services = strip_extension(
                pkg_resources.resource_listdir(__name__, "services"))
        except OSError:
            raise InvalidServiceDirectory(servicedirectory)
    ...
    sys.path.insert(0, servicedirectory)

    for Service in Services:
        if Services_to_load and Service not in Services_to_load:
            logger.debug("skipping Service {0}, not in Services_to_load {1}".format(Service, Services_to_load))
            continue
        ...
        try:
            ...
            for hook in re.findall(r"\bon_(\w+)", " ".join(dir(mod))):
                ...
                logger.debug("Service: attaching %s hook for %s", hook, modname)
        ...
        # on import, and we want them not to kill our server
            ...
            logger.warning("import failed on module {0}, module not loaded".format(Service))
...
def run_hook(hooks, hook, *args):
    ...
    for hook in hooks.get(hook, []):
        try:
            ...
            logger.warning("Failed to run Service {0}, module not loaded".format(hook))
            logger.warning("{0}".format(sys.exc_info()[0]))
            logger.warning("{0}".format(traceback.format_exc()))
...
def init_config():
    ...
    getif(config, "loglevel", "FABULOUS_LOGLEVEL")
    getif(config, "logfile", "FABULOUS_LOGFILE")
    getif(config, "logformat", "FABULOUS_LOGFORMAT")
    getif(config, "Services", "FABULOUS_ServiceS")
...
def loop(server, test_loop=None):
    ...
    server is a Fabulous Server object
    ...
    try:
        ...
        if os.environ.get("FABULOUS_DEBUG"):
            import ipdb
            ipdb.set_trace()
        raise
...
def relevant_environ():
    return dict((key, os.environ[key])
                ...
                if key.startswith("SLACK") or key.startswith("FABULOUS"))


def init_server(args, config, Server=FabulousServer, Client=SlackClient):
    init_logger(config)
    logger.debug("config: {0}".format(config))
    db = init_db(args.database_name)

    config_services = config.get("Services")
    services_to_load = config_services.split(",") if config_services else []

    hooks = init_services(args.servicepath, services_to_load)
    try:
        slack = Client(config["token"])
    except KeyError:
        logger.error("""Unable to find a slack token. The environment variables
fabulous can see are:
{0}
...
def main(args):
    ...
    if args.test:
        ...
        print(run_cmd(cmd, FakeServer(), args.hook, args.servicepath, config.get("Services")))
        return
...
def run_cmd(cmd, server, hook, servicepath, services_to_load):
    server.hooks = init_services(servicepath, services_to_load)
...
def repl(server, args):
    try:
        while 1:
            cmd = decode(input("fabulous> "))
            if cmd.lower() == "quit" or cmd.lower() == "exit":
                return

            print(run_cmd(cmd, server, args.hook, args.servicepath, None))
